...
void Gizmos::addSphere(const glm::vec3& a_center, int a_rows, int a_columns, float a_radius, bool aWireFrame, const glm::vec4& a_fillColour,
...
void Gizmos::addCone(const glm::vec4 aPosition, const glm::vec4& aLightDir, int aSegments, float aLength, float aInnerCutoff, float aOuterCutoff,
					bool aFill, const glm::vec4& aColour)
{
	glm::vec4 originPoint = aPosition;
	glm::vec4 endPoint = originPoint + (aLightDir * aLength);
	...
	for (unsigned int i = 1; i < segments; ++i)
	{
		...
		v1 += glm::vec4(angleCos, 0.0f, angleSin, 0.0f) * aOuterCutoff;
		...
		v2 += glm::vec4(angleCos, 0.0f, angleSin, 0.0f) * aOuterCutoff;

		if (aFill)
		{
			addLine(originPoint, v1, aColour);
			addLine(v1, v2, aColour);
			addLine(v2, originPoint, aColour);

			addTri(v1, originPoint, v2, aColour);
			addTri(endPoint, v1, v2, aColour);
		}
		else
		{
			addLine(originPoint, v1, aColour);
