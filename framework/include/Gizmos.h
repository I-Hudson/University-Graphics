...
public:
	...
	static void		addSphere(const glm::vec3& a_center, int a_rows, int a_columns, float a_radius, bool aWireFrame, const glm::vec4& a_fillColour,
							  const glm::mat4* a_transform = nullptr, float a_longMin = 0.f, float a_longMax = 360, 
							  float a_latMin = -90, float a_latMax = 90,
							  glm::vec3** vertexData = nullptr, unsigned int* a_vertexCount = nullptr);

	static void addCone(const glm::vec4 aPosition, const glm::vec4& aLightDir, int aSegments, float aLength, float aInnerCutoff, float aOuterCutoff,
