...
// This needs to be used along with a BaseRenderer (e.g. OpenGL3, Vulkan..)
...
{
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally built by the BaseRenderer back-end. Missing call to BaseRenderer _NewFrame() function? e.g. ImGui_ImplOpenGL3_NewFrame().");
...
//--------------------------------------------------------------------------------------------------------
// Vulkan support (the Vulkan BaseRenderer needs to call a platform-side support function to create the surface)
//--------------------------------------------------------------------------------------------------------
...
static void ImGui_ImplGlfw_InitPlatformInterface()
{
    // Register platform interface (will be coupled with a BaseRenderer interface)
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
