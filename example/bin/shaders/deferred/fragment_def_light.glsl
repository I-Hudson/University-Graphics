...
float reMap(float aV, float aStart1, float aStop1, float aStart2, float aStop2)
{
...
vec4 CalcPointLight(PLight aLight, vec3 aWPos, vec3 aNormal)
{
	...
	return vec4(radius, radius, radius, 1.0) * aLight.Intensity;
}
...
vec4 CalcSpotLight(SLight aLight, vec3 aWPos, vec3 aNormal)
{
	...
	vec3 specular = vec3(1, 1, 1) * spec; // assuming bright white light color
